...
- Make every Skill self-contained: a Skill must work when installed as a copy on its own. No Skill file may reference a repository directory or repository-root file (`docs/`, `tools/`, `cmd/`, `internal/`, `bin/`, `Makefile`, `go.mod`, `skills/<skill>/...`, or `../../docs/...` links) — those do not exist in an installed copy. Keep every script a Skill runs inside that Skill's own `scripts/` (with its tests, schemas, examples and README); reference a sibling Skill's script as "the `<skill>` Skill's `scripts/<file>`", never as a repo path; take CLI usage from `crwu scheme` rather than a repository manual; derive a script's skills root from `__file__`, never from repository layout. Only declared source-repo contract tests may locate the repository, and they must skip explicitly when it is absent. Full rule, the two allowed exceptions and the machine gate (`kb_tool.py validate`) are in `skills/AGENTS.md` §「技能自洽性」.
