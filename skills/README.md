...
| [`crwu-audit-external-data`](crwu-audit-external-data/SKILL.md) | 提示型 | 跨方向 **L2 外部数据核验**（public 轴插拔）：`methods[]` 命中收益法/市场法时由 router 叠加；按知识库 `M-外部数据核验` 模块规程（表 A 触发范围 / 表 B 数据项触及 / 表 C 组合映射 / 表 D 万得条件与降级）核验报告引用的外部公开数据——经宿主连接器（同花顺 iFinD 主源、万得副源）取数，口径标准化后比对，逐项出【符合 / 不符合 / 请说明 / 未检查】并标明与哪一源出入较大；**所有取数以报告基准日为锚，禁用取数时点滚动窗口**；连接器缺失/未启用/未认证时按表 D 降级并在交付 HTML《外部数据核验》区显式声明；`scripts/connector_probe.py` 只读探测宿主连接器（不读凭据值） |
