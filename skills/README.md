...
| [`crwu-audit`](crwu-audit/SKILL.md) | 提示型 | 报告审核能力族**总路由**（≤300 行入口 + `references/` 运行材料）：维护全部 crwu-audit-* 路由注册表；画像=报告形态→经济行为主线（受控角度词表）→对象→方法（附件名+抽验）→监管覆盖层（国资/证券/司法/金融）；字段画像先行、弱结构化名称兜底；细分未命中降级父大方向→汇总输出（自身不做审核判断）；**兜底/🅿️⏳ 命中时自动附《待建子技能提案》**（references/04：补哪个子技能/管什么/审什么）；画像必含 **scenario 装配键**（评估目的×方法双键，refs/00 §3 / 01）；**两阶段输出：阶段一独立审核（意见定稿前不下载/读取/参考复核记录）→ 阶段二复核对照 A/B/C + 综合对比（AI∩复核/AI 新增/复核独有=漏检候选）**；交付按技能内 **`references/11-html-delivery-spec.md`（CRWU 审核意见 HTML 送达规范 v1.4）**：AuditResult JSON 单一事实源 + **JSON 校验先行，合规后才成对生成员工 HTML 与同源监控 JSON** + 双证据链 + 《本次审核记录清单》+ **问题描述强制「员工话」两段式**（首句 ≤60 字说清问题是什么 + 2–4 行明细写明差异与去哪个文件/sheet/单元格核对；不得出现规则编号、知识库路径与内部代号，`validate` 硬校验拦截）+ **每个项目向员工只交付一个自包含单文件 HTML `审核意见.<项目ID>.html`（可离线、A4 可打印、含折叠专业审核轨迹），内部 JSON 与 HTML 内嵌 AuditResult 完全一致**；材料准备经技能内 `scripts/prepare_materials.py` 重建只含可见区的**工作版**，并把**媒体（图片）证据**导出到 `媒体证据/`、写 `媒体索引.json` 放行清单（锚点落在隐藏区者按 H0 不导出；读不到记未核，**未核 ≠ 缺失**）；阶段二复核件以 `--src 复核-人工 --label 复核` 复跑以抽取复核意见附件里的图，且不覆盖阶段一冻结产物；改前先读 `references/99-维护说明.md` |
...
**族维护分工**：`crwu-dev-audit-optimize` 负责从漏检、误检和复核反馈诊断“需要改什么”，并交付可视化差距报告；知识库修复单只供用户手工执行，Skill 修改须逐项确认。`crwu-audit-skill-maintainer` 负责盘点目录树以及创建、修复、重映射资产/业务 Skill。两者都不经 crwu-audit 路由、不产审核判断。
