...
目录存在不等于内容已达到可执行规则状态。`06-规则库/M-收益法/` 当前登记为 `prototype/non-runtime`：其模块规程仍为原型，规则依据含“待编”占位，不能作为正式审核依据。检查器必须在 JSON、文本报告和校准表中显式列出该路径，但不得把它计作运行时装配缺口，也不得因为“目录已识别”而把它加入 router。

如果任何装配表或 router 将该目录加入运行时，记 `NON_RUNTIME_MODULE_ASSEMBLED`（error）。只有知识库将状态改为正式、补齐规则编号并明确维护人，且维护者重新读取正文、完成人工复核后，才能移除隔离并设计正式映射；目录内容自行变化不会自动启用。
