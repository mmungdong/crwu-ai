...
## 方法层、覆盖层与财务报告补充规则的库内装配映射（2026-09-16 立规）
...
财务报告覆盖规则不是监管主体 overlay：其触发条件是已经存在的 `business_types[]=财务报告`，不得虚构同名 overlay 才能装配。

**规则**：本次 DWS 下载清单在「命中叶子装配表 ∪ 公共执行契约」之外，**再按本次 `methods[]`、`overlays[]`、下表明确列出的 `business_types[]`，以及显式登记的跨轴条件命中项追加目录**。
**只装命中项，不装全集**——未命中标签的层不得下载，避免把不适用规则装进本次审核。
跨轴专项映射是在单轴共性映射基础上追加，不替代单轴共性资料；同一项目同时命中单轴与跨轴行时，两行路径取稳定并集。

| 轴 | canonical 标签（命中即追加） | 库内层级路径（目录项以 `/` 结尾、递归下载） |
| --- | --- | --- |
| method | `收益法` | `03-评估方法/02-收益法/` |
| method+asset | `收益法` ∧ `企业价值` | `06-规则库/清单-M-收益法/` |
| method | `资产基础法` | `03-评估方法/00-评估方法准则2019-精编/评估方法准则2019-精编条目/04-成本法`、`03-评估方法/03-资产基础法/` |
| method+asset | `资产基础法` ∧ `房地产` | `06-规则库/清单-M-资产基础法/` |
| method | `成本法` | `03-评估方法/00-评估方法准则2019-精编/评估方法准则2019-精编条目/04-成本法` |
| method+asset | `成本法` ∧ `房地产` | `06-规则库/清单-M-成本法/` |
| method | 任一方法命中（选择、差异处理与跨方法缺陷为共性事项） | `03-评估方法/04-方法选择/`、`03-评估方法/05-审核要点/` |
| method | `市场法` | `03-评估方法/00-评估方法准则2019-精编/评估方法准则2019-精编条目/02-市场法`、`03-评估方法/01-市场法/` |
| method+asset | `市场法` ∧ `房地产` | `06-规则库/清单-M-市场法/` |
| business | `财务报告` | `04-监管覆盖/财务报告/` |
...
1. **标签触发与目录装配映射的唯一事实源在本节**；叶子可在审核关注点中记录实际规则文件路径用于执行和出处，但不得自行改变触发标签、目录范围或装配行为。
...
3. **清单外零下载**不变：映射只按单轴已确认标签或表内明确登记的跨轴条件追加。`method+asset` 行须两侧标签同时命中；不得凭对象名称、行业惯例或未登记业务推断。
4. 市场法通用资料、跨方法缺陷与房地产方法专项清单均由本节按实际画像装配；资产叶子不得无条件重复持有。`06-规则库/易错点库/`
   由公共通用准则技能恒装配。`06-规则库/清单-M-成本法/` 与 `06-规则库/清单-M-资产基础法/` 分别只由本节对应方法 ∧ `房地产` 的跨轴映射装配，两份清单不得互相替代
   （去重由 `stable_unique` 与清单并集完成）。
