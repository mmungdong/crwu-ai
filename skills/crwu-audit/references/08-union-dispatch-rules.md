...
**规则**：本次 DWS 下载清单在「命中叶子装配表 ∪ 公共执行契约」之外，**再按本次 `methods[]`、`overlays[]` 与下表明确列出的 `business_types[]` 命中项追加目录**。
...
3. **清单外零下载**不变：映射只按 `methods[]`、`overlays[]` 或表内明确登记的 `business_types[]=财务报告` 追加，不做模糊扩展、不按对象名称或未登记业务推断。
