...
资产基础法即成本法项下的成本加和法，不是与成本法并列的第四种基本评估方法。为保留报告原文、证据定位和专项清单选择的准确性，`methods[]` 中仍将 `资产基础法` 与直接表述为 `成本法`、`重置成本法` 的方法保留独立 canonical 标签。

识别到 `资产基础法` 时，装配层必须继承成本法共性准则，同时装配资产基础法说明；仅在 `asset_types[]` 同时命中 `房地产` 时追加资产基础法不动产专项清单。不得据此新增一个 `成本法` 标签，也不得装配 `06-规则库/清单-M-成本法/`。只有报告真实证据独立表明还采用了成本法时，才可另记 `成本法` 标签并按其映射装配。两个专项清单不得互相替代。
