...
7. **先判公共能力**：在求并集前独立求值每个公共能力，各自按自己的触发条件判定——**通用准则类（`crwu-audit-public-general-standards`：报告披露层 + 程序质控层）与审核意见输出过滤（`crwu-audit-output-filter`）均与报告形态、对象、业务、方法、监管无关，无条件写入 `public_skills[]`**；输出过滤技能此时只加载并装配清单，延后到候选意见汇总后、阶段一冻结前执行。表格类在材料含测算、明细、汇总等表格时写入，否则不写；外部数据核验必须先按知识库 `M-外部数据核验` 表 A 判断业务线 × 资产线 × 方法线是否同时成立，并把结论写入 `externalDataVerification.applicability`。未同时成立时记 `not_applicable`，不得探测数据源、不得记未检查项或能力缺口；同时成立时才加载 `crwu-audit-external-data` 并探测同花顺 iFinD，只有此时能力不可用才在交付 HTML 显式声明未配置/未认证。即使所有专业标签都为 `pending`，其他公共能力仍须独立求值并加载。
8. **完整稳定并集**：六个候选数组全部求值后，严格按 08 计算 `skills_to_load`。不得 first-match、不得使用排他链、不得以后加载技能覆盖先前轴，也不得在并集或执行后追加 public skill。
9. **先准备完整 DWS 清单**：汇总全部 `skills_to_load`（包括 `public_skills[]`）各自 references 声明的层级路径，与两个执行契约 `00-总纲/执行契约/02-防幻觉协议执行细则`、`00-总纲/执行契约/03-审核统计与台账规范`，**再按本次 `methods[]`、`overlays[]`、明确登记的 `business_types[]=财务报告` 及映射表中的显式跨轴条件，追加方法层、覆盖层与财务报告补充规则的库内目录（映射唯一事实源见 `references/08-union-dispatch-rules.md` §「方法层、覆盖层与财务报告补充规则的库内装配映射」，只装命中项、不装全集）**，去重形成本次 DWS 清单（**交付/送达口径不再走知识库契约，见 `references/11-html-delivery-spec.md`**）。在任何技能执行前，对整份清单逐项完成本次 `crwu-dws` 实时下载和成功/失败验证；目录缓存不能提供正文。下载文件、行号、库内路径、运行时 `nodeId` 与 `exportedAt` 只作本次证据，不写回 Skill source。
10. **同时加载执行**：完整清单验证结束后，同时加载并执行规则材料验证成功的 `skills_to_load`（包括 public skill），但 `crwu-audit-output-filter` 只加载其运行时清单，不在专业技能执行阶段提前过滤检查项。验证失败的技能仍保留在 `dispatch.skills_to_load` 及执行状态中并记录 gap，不得静默删除，也不得短路其他成功技能。每个叶子只接收同一 `route_profile`、已准备的阶段一源材料工作路径，以及经验证的本次 DWS 规则材料路径/manifest；不得接收复核记录，也不得重复 fetch 记录。
11. **阶段一独立汇总、过滤并冻结**（并产出**AI 自查记录**）：先汇总所有专业与公共审核能力基于源材料独立产生的 findings，按报告、评估说明、测算明细表分区去重。每条 finding 必须保留 `source_skills[]`；相同结论合并时保留全部来源，冲突结论并列留待人工复核，不相互覆盖。**候选审核意见汇总完成后、阶段一冻结前**，执行 `crwu-audit-output-filter`：只按本次实时下载《02-屏蔽清单》中启用记录的 `匹配标识` 精确匹配，并同时核对适用范围、屏蔽条件和不屏蔽例外；命中全部条件者不进入正式审核结果，未命中、冲突或无法判断者保留。清单缺失或失败时零屏蔽并记 gap，不得沿用旧清单或模糊匹配。在读取任何复核记录前，定稿并冻结过滤后的 AI 正式意见、完整 `route_profile`、全部命中轴，以及本次完整规则/模块清单；这些阶段一产物在阶段二不可改写。**冻结指纹按本技能 `scripts/audit_delivery.py digest <冻结快照.json>` 计算**（规范化序列化 sha256）写入 `phaseControl.phase1FrozenAt` / `phaseControl.phase1Digest`，冻结后才允许进入阶段二。同批产出 **`selfAuditErrors[]`（初审自查发现的 AI 自身错误：假阳性/事实更正/漏检/表述，记 `discoveredAt=初审`）**；已关闭的 `false_positive` 不得仍作为有效问题保留。旧版 `aiScorecard` 六维自评仅为历史兼容字段，不得进入员工端客观评分卡或替代复核命中率。**同批完成媒体证据判读**：逐条消费 `媒体索引.json`（`locator` 逐字引用），用宿主多模态读图（DeepSeek Harness `read_image`）把读到的事实写入对应材料证据的 `excerptOrValue`；宿主无视觉能力、图片不可读或媒体未导出时逐条记 `scope.notCheckedItems`（`reasonCode=unreadable|unsupported`）并给 `requiredAction`——**未核 ≠ 缺失**，严禁写成"为空/缺失/未列示"。
...
15. **钉钉结果回传**：步骤 14 的最终态 `审核结果.<项目ID>.json` 生成后，按 `references/13-dingtalk-result-publish.md` 执行 `python3 scripts/upload_audit_result.py 审核结果.<项目ID>.json`。只精确使用组织 `中瑞世联资产评估集团有限公司` 下的固定团队空间与结果根目录；仅允许在结果根目录内按 `auditTask.auditTime` 自动创建缺失的 `YYYY/MM` 两级目录。远端文件名追加 `fileTrace.generatedAt` 时间戳，已存在同名文件时拒绝覆盖。上传失败保留本地交付件并显式报告“钉钉回传失败”，不得把审核完成等同于回传成功。
...
装配清单由叶子声明、不经本仓装配器：命中轴标签后，下载清单 = 命中叶子 `references/01-kb-assembly.md` 的路径键（一级目录根或单文件路径）＋公共执行契约路径并集 ＋ **按本次单轴标签和映射表显式跨轴条件命中的方法层、覆盖层与财务报告补充规则目录**（映射表见 `references/08-union-dispatch-rules.md`），由 `crwu-dws` 实时下载正文。叶子的一级目录根按 `request_kind=directory`、`recursive=true` 递归下载根内全部支持正文；细分对象与子业务在已下载目录包内二次选用，不各建技能。**方法轴与覆盖层技能为 `pending` 期间，其知识库内容仍必须按本条的映射命中项装载**——`pending` 表示能力未落地，不表示内容可以不装；财务报告补充规则由 available 的财务报告业务技能执行，不产生额外 overlay gap。下载失败、空目录或不支持导出的节点记 capability gap，并继续执行不依赖该步骤的已加载能力。
