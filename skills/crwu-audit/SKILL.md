...
- 本族自含分发逻辑与运行材料，**与知识库仓库解耦**：不预读知识库入口文档、不引用其路径；规则正文与输出/统计/防幻觉契约经 crwu-dws 按库内层级路径实时下载后只读引用（部署注入的是 crwu-dws/dws 执行环境，不是本地静态副本）
...
5. **方法（路由阶段从文件提取，必须走真实读取链路）**：前置三步——① 下载附件
   ...
   **红线：评估目的/方法/结论方法及 loc 只能来自真实读取的文件——严禁凭字段、附件文件名、
   ...
   （references/00-KB装配表.md）的并集，见步骤 10——正文一律实时下载后引用，不读本地静态副本。）
...
10. **汇总输出**：意见按 报告/评估说明/测算明细表 分区去重；附路由路径与**调用链**（本次执行/拟执行的
    ...
    route_profile 摘要（含 显性输入/目的·方法文件级提取/complexity）与三段式引用（输出/防幻觉契约
    `00-总纲/执行契约/`（目录级：01~04 与 README 全量下载，含 02-输出Schema与审核意见单.md、03-防幻觉协议执行细则.md）
    经 crwu-dws 实时下载后只读引用，禁止放宽；知识库正文不预读本地静态副本）。材料含人工复核意见（一/二/三级/
    质控意见、答复文件）时，附《复核意见对照表》（分类与红线见契约 02 §5）：已在复核意见提出的 →
    指出 出处文件+条目/页码+原文摘录；复核意见未提出的 AI 新增发现 → 单列并建议人工复核补充；
    复核材料缺失/不可读 → 显式“未对照”，禁止编造出处；
11. **统计回填**：route 命中/降级/空能力/weak_structured 计数 → `00-总纲/执行契约/`（目录级全量下载中的 04-审核统计与台账规范.md）台账（经 crwu-dws 实时下载后引用，不读本地静态副本）。
