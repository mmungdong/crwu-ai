...
| 用户意图（例） | 模式 | 主要产物 |
| --- | --- | --- |
| "看/查/列 中瑞世联评估审核知识库 的 目录/层级/树/结构" | **M1 目录查询** | 摘要 + 目录缓存（`~/.crwu/knowledge/dws-dir-cache/<库名>/`：`目录快照.json`/`目录树.md`/`node-index.json`/`.cache-meta.json`） |
| 审核编排：路由/装配已定文件清单，"下载本次审核需要的知识文档（最新实时版）" | **M2 按清单实时下载**（审核消费主路径） | `<案例目录>/knowledge/` 本次工作集（逐文件 exportedAt）+ `.crwu-manifest.jsonl` + `.crwu-directory.json` |
| "找/查 文件 XX / 层级路径 …（在不在库里、哪个目录）"、"nodeId … 是什么"、"取 XX 最新内容" | **M3 缓存兜底查找（含路径寻址）** | 命中：元数据+树路径（要正文 → 实时拉取入本次工作目录/即弃）；未命中→刷新→仍无："未找到"报告 |

- 目标库名默认「中瑞世联评估审核知识库」；其他库名同流程处理（产物目录以实际库名为准）。
- **缓存名一致性（清理层，§5.0）**：缓存目录按 `<库名>` 分目录、只对与 skill 目标库名一致的库有效；缓存库名与目标不一致（改名/换库/历史残留）→ **先清理该缓存目录，再在线重下目标库目录结构**；禁止把别的库的缓存当目标库索引（跨库误命中）。
- **不触发**：泛化钉钉知识库/doc 管理（→ `dingtalk-wiki`/`dingtalk-doc`）；审核报告材料（→ `crwu-audit`）。
- M2 由 crwu-audit 编排者在路由/装配确定本次文件清单后调用（清单 = 命中子技能装配路径表 + 执行契约路径并集）；M3 供 skill（audit 族等）定位并现场获取参考文档，也可对用户直接服务。
...
- **对账基准 = 本次目标库名**（skill 默认「中瑞世联评估审核知识库」或本次指定库名；M1 与 M3 刷新路径叠加 P1 解析结果，按 name+workspaceId **双匹配**对账；M3 **命中路径不发起在线遍历**（§7.1 约定），以请求库名为准做名级对账——workspaceId 一致性在每次写缓存时已由 P1 校验并固化于 `.cache-meta.json.space`）。
...
- **清理动作**：删除不一致缓存目录（正式 4 文件 + 任何残留 `.tmp-*`）。目录缓存 = 可再生中间产物、不含正文 → 删除安全，红线不受影响；只作用于 `dws-dir-cache`，不碰 M2 当前审核 `knowledge/` 工作集或远端。删除失败 → 报告路径问题，该缓存按不可用处理，不静默复用。
- **重建**：对目标库在线重跑 P1+P2 全量遍历，重新下载远程目录结构并原子写缓存（即本 § 下方 1–4 写缓存流程）；M3 侧进入 §7.2 刷新路径后重查。
- **报告**：清理动作必须可见——"已清理不一致缓存：<目录>（缓存=<库名>，目标=<库名>）"；重建后附新 generated_at。
...
## 6. P3-M2 按本次清单下载（远端只读 + 本地写）
### 6.1 目标目录决议（禁止猜）
`knowledge/` 的父目录 = 案例目录（= crwu-audit 源审核数据文件所在目录的父级，二者同级）：① 编排者/用户显式给出；② 部署约定 `CRWU_CASE_DIR`（若注入的是材料包目录则取其父级）；③ 都没有 → 询问用户。

### 6.2 M2 按清单实时下载（审核消费主路径；文件零缓存 R2/R3）
输入：本次审核文件清单 = 层级路径列表 `paths[]`，来自 crwu-audit 路由/子技能装配路径表；**同一清单**可混合以下两种条目：

- **单文件路径**（不以 `/` 结尾，如 `<目录>/<文档>.md`）：精确解析并**只下载该文件**。
- **目录路径**（以 `/` 结尾，如 `<目录>/`）：展开并**递归下载**该目录下全部支持的正文文件。
...
输入：精确文件名 或 nodeId 或 **库内层级路径（文件级或目录前缀）**。
...
1. **查缓存**：读 `<缓存>/node-index.json`（nodeId → 条目；名称 → 条目列表）：
   - 命中（nodeId/名称/路径任一键）→ 报告：条目元数据（type/parentFolderId/所在库）+ **树路径上下文**（顶层 folder 链），结论标注"缓存命中（generated_at=<…>）"；不发起在线遍历；
   - 名称/路径多命中（同名不同路径等）→ 全部列出路径上下文，请调用方/用户消歧（不猜）；
   - **路径寻址**：全路径（`目录/…/文档`）精确命中 → 同上；目录前缀（`目录/…/`）→ 该目录下条目列表（限定层级展示）；未命中 → 进入刷新（node-index 全路径键见 references/02 §3）；
...
5. **刷新失败降级**：在线遍历失败（认证/网络/分页未完成）→ 旧缓存可用：用旧缓存作答并显式标注"**缓存可能过期（generated_at=<…>），刷新失败：<原因>**"；无旧缓存 → 报告"无法确认（缓存缺失且在线刷新失败）"。

## 8. P4 一致性自查
- **写缓存前先校验快照 schema（硬门禁，不通过不得写、不得宣称全量）**：`目录快照.json` 的
  `schema` 必须逐字等于 `crwu.kb-catalog.snapshot.v1`；`generated_at` / `profile` / `mode` /
  `space` / `stats.complete` / `nodes` / `failures` 齐备；`nodes` 为**嵌套 `children`**
  （扁平自带 `path` 的是 `node-index.json` 形态，写错会让映射检查器直接拒收整份缓存）。
  三个 schema 名与时间字段名不得混用（见 references/00 §3）。
- M1/M3 刷新：快照节点计数 == 遍历完成计数且 `failures=[]` 才写缓存/宣称全量；缓存 meta 的 `last_successful_at` 与快照 `generated_at` 一致。
...
- **红线（用户口径 R1–R5）**：目录元数据可缓存、**知识正文不缓存**；知识正文唯一来源是钉钉知识库，每次审核都按本次清单现场下载。本次下载物只属本次审核，跨审核一律重新下载（R2）；只下载单文件命中项或目录展开项，**清单外零下载**（R3）。
- 目录缓存只用于查找加速，不能提供正文或成为正文兜底。差异由人工/复核判定。
- 变更登记纪律：只改源仓并提交 + 源仓变更纪要；运行时部署由用户 skills 管理机制负责。
