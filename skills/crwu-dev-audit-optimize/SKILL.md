...
description: >-
  ...
  审核结束后，如同时存在 AI 审核结果与人工复核文件，也用于执行 AI—人工差距分析：逐条识别仅人工发现项，
  追溯到知识库、Skill、路由装配、执行或输出层，生成可视化 HTML 差距报告、修复计划与可复制提示词；
...
3. 改动只落在 references/00 §2 分层地图对应的**唯一落点文件**并提交源仓；运行时部署由用户 skills
   ...
   必须同批完成"装配表/知识库登记/叶子引用"三件套同步**（判定见 references/00 §2.1）：
   新增、改名、移动或调整装配范围 → 同步叶子 `references/01-kb-assembly.md` 与知识库 `00-总纲/README`（总纲与维护契约；该库当前缺 `目录地图`，见 references/00 §2.1 gap）；
   知识库正文编辑本身按知识库维护流程另办——禁止留下"文件已动、指针没跟"的中间态。
4. 引用协议（与 crwu-audit 族实时引用协议一致，见 design-audit-live-kb-protocol §2 R1–R5）：技能正文引用
   一律写 RULE/CHK 编号 + 库内层级路径寻址键（如 `00-总纲/执行契约/01-调度器-SKILL总纲`、`06-规则库/…`），不写知识库名称、
...
5. **知识库只读、修复必须人工执行**：本技能可通过 `crwu-dws` 只读下载知识库正文用于诊断，但所有知识库
   修复计划必须标记 `manual_only`，只输出目标文档、证据、建议修改位置与人工操作说明，**禁止修改知识库**；
   AI 只能在用户明确批准具体 `FIX-*` 后修改源仓 Skill 文件，批准不得从一次差距分析自动继承。

## 0.1 模式选择

- **AI—人工差距分析模式**：已有同一次审核的 AI 结果与人工复核文件时，读
  `references/03-AI人工差距分析流程.md`，输出表格 + ECharts 的单文件 HTML 报告。
- **常规反馈优化模式**：只有用户反馈或缺口描述时，继续按 §1–§5 定位、分桶、出方案。
- **受控执行模式**：只有用户明确批准具体修复项后才进入执行；知识库项始终由用户人工处理，AI 不执行。
...
3. 若存在历史审核产物（审核意见单 json/md、路由路径、适用规则集快照），先读——它就是"现状基线"，
   证明当前命中面与真实审核的差异。
4. 机器辅助定位（调用 `crwu-dev-audit-skill-maintainer` 技能的 `scripts/kb_tool.py validate`；两技能安装在同一 skills 根下，下同）：
   - `python3 "$SKILLS_ROOT/crwu-dev-audit-skill-maintainer/scripts/kb_tool.py" validate --skill-root <源仓 skills> --skill-root <运行时真身 skills>` → 引用卫生与漂移线索
...
5. 人工对照（知识库治理/目录文件经 crwu-dws 按库内层级路径实时下载后只读核对）：
   `00-总纲/README`（知识库总纲与维护契约：§2 目录、§3 三处登记要求）、叶子 `references/01-kb-assembly.md`（技能→知识库寻址键）、
   ...
   maintainer 的实时校准表 `crwu-dev-audit-skill-maintainer/references/07-kb-skill-map.md`（知识库↔Skill 映射现状）。
6. 输出一行定位小结：画像五维 + 现行路由路径 + 命中 M/排除 N + 疑似缺口 X（供用户先纠偏画像）。

## 2. 诊断分桶（根因 → 谁来做）

**职责分界（2026-09-10 重划）**：本技能只做**诊断**（"为什么要改、改哪一层"），并**只亲自执行内容层与算法层**的改动；凡是**结构与映射层**的改动一律交接 `crwu-dev-audit-skill-maintainer`，本技能不自行改 skill 结构、registry、classification。

| 桶 | 症状 | 落点 | 谁执行 |
| --- | --- | --- | --- |
| **A 规则/清单内容** | 依据缺失、口径错误、检查点漏项 | 知识库 `06-规则库/`（精编条目 / `M-*` / `清单-M-*`） | 本技能诊断并生成人工修复单；**用户手动修改知识库** |
| **C 词表/画像取值** | `object_type`/`method`/`scenario`/`stage`/`report_type`/角度 新取值未登记 | `00-总纲/治理/标签词典` + crwu-audit `references/00`/`01` | 知识库部分人工修复；Skill 部分经逐项批准后由本技能执行 |
...
- 本技能回答"**为什么要改、应改哪一层**"；`crwu-dev-audit-skill-maintainer` 回答"**缺哪些一级 Skill、哪些映射失效、逐文件怎么改**"。
...
**不在此重复定义**：两阶段门禁（只读方案 → 用户确认 → 只改批准文件）、逐文件方案必答项、校验与回归命令，一律沿用 `crwu-dev-audit-skill-maintainer/references/04-registry-and-mapping-update.md` 与 `references/05-validation-and-delivery.md`；本技能 references/02 只保留优化方案特有的模板差异。
...
差距分析先生成结构化 JSON，再用固定模板渲染；报告必须把每个仅人工发现项 `L-*` 映射到根因及
一个或多个 `FIX-*`，修复项再以 `sourceGapIds[]` 反向指回来源。命令：
...
HTML 必须包含汇总表、ECharts 图、`L → 根因 → FIX` 关系、知识库人工修复单、经批准的 Skill 修复提示词、
执行轨迹和验证计划；详细判定、双基线与状态口径见 `references/03-AI人工差距分析流程.md`。

## 4. 执行（仅在确认后）

1. 只执行用户明确批准的源仓 Skill `FIX-*`，按 references/00 §2 顺序改文件（先落点文件、后引用它的上层文件）；
   知识库 `FIX-*` 仅交付 `manual_only` 人工修复单，不执行、不通过 CLI 写入；
2. 源仓提交；运行时部署/校验由用户 skills 管理机制执行（本侧不直接写运行时目录）；
3. 登记：源仓的设计文档、技能清单与变更纪要（99 流程；改词表另提示复跑画像基线）；
   涉及知识库规则文件的改动同步更新批次 README/进度总表对应行；
4. 校验回归（**全过才算完成**）：
   - `python3 "$SKILLS_ROOT/crwu-dev-audit-skill-maintainer/scripts/kb_tool.py" validate --skill-root <源仓 skills> --skill-root <运行时真身 skills>` error=0
...
- 新增审核叶子能力属 **B 桶 → 交接 `crwu-dev-audit-skill-maintainer`**（`create`）：三处登记
