...
Go 1.24+ · CLI-first · H3Yun employee-scope reads · MCP (planned)
...
CRWU is a Go command-line harness that gives AI hosts (WorkBuddy, DeepSeek
Harness, operator terminals) a stable, auditable way to reach enterprise
systems. It ships one executable — `crwu` — with a machine-readable command
...
- Each employee scans a QR at `h3yun.com` with DingTalk, then binds the
  resulting **web session** to this machine once (48 h, refreshable).
- Every command afterwards runs **under that employee's permissions** —
  applications, forms, records and attachments are exactly what they can see
  in the browser. No passwords, no cross-employee data, no server to operate.

> **Status:** session binding, app/form/record browsing and attachment
> downloads are working. Agent-gateway (`h3pat`) tools, the MCP transport and
> Python Skills are on the roadmap (see [Channels](#channels--two-ways-in)).
...
file is required to start; optional `H3YUN_BASE_URL` overrides the H3Yun
console origin for testing.

## Employee workflow

**1 · Bind the session once** — ask the employee to scan at `h3yun.com` and
copy the bearer JWT from the browser (DevTools → Network → `Authorization`):
...
> chat. Sessions expire after 48 hours; renew with `session refresh` or
> re-bind after a fresh scan.
...
| `crwu h3yun file download --schema <code> --id <id> --out <dir>` | Download all attachments, deduplicated, with original names |
...
`crwu scheme` writes **only JSON** to standard output, so an AI client can
parse the full catalog (version, descriptions, usages, examples) without
stripping log noise; diagnostics go to stderr with a non-zero exit code.

Follow [`docs/cli-command-contract.md`](docs/cli-command-contract.md) before
...
- **Transports** own parsing and presentation (CLI now, MCP later).
- **Application services** orchestrate host-neutral use cases
  (`internal/app/h3yunops`, `internal/app/h3yunweb`).
- **Integrations** own provider protocols only — the H3Yun web REST client and
  the H3Yun agent MCP client live together under
  `internal/integrations/h3yun` and never import another provider.
...
internal/
  ...
  app/h3yunweb/                 web-session application service (bind/refresh/reads/files)
