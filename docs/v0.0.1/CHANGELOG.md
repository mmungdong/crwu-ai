...
- **固定目标门禁**：仅精确匹配组织 `中瑞世联资产评估集团有限公司`、团队空间
  `00-【系统专用】AI结果回传区（自动同步·请勿删改）` 与结果根目录 `AI资产评估审核结果`；
  零命中、多命中或类型不符时停止，不跨组织猜测。
- **年月归档**：按 `auditTask.auditTime` 建立 `YYYY/MM`；只有这两级目录允许缺失后自动创建，固定目标层不得创建。
- **重名保护**：远端文件名为 `审核结果.<项目ID>.<fileTrace.generatedAt时间戳>.json`；同名存在时拒绝覆盖。
- **实现与验证**：新增 `scripts/upload_audit_result.py` 及契约测试；上传前校验最终态 JSON，上传后读回名称、节点 ID
  与可用的文件大小证据。失败保留本地交付件并显式报告，不把审核完成误报为远端回传成功。

## 2026-09-18 · fix(crwu-audit) · 外部数据核验适用性前置判定（送达规范 v1.6 / renderer 1.2.6）

- **适用性优先**：AuditResult 新增必填的 `externalDataVerification.applicability`，按知识库
  `06-规则库/M-外部数据核验/01-模块-外部数据核验` 表 A 记录 `required` / `not_applicable`、原因与依据。
- **避免误报**：不适用时 HTML 明确显示“本报告暂不涉及外部数据核验”，不探测或告警数据源，也不记未检查项、
  capability gap；仅在确需核验但同花顺 iFinD 不可用时显示未配置/未认证声明。
- **契约升级**：送达规范升至 v1.6、renderer 升至 `renderer/1.2.6`、`schemaVersion` 升至 `1.2.0`；
  JSON 校验、员工 HTML 与后续监控统计保持同一结构化口径。

## 2026-09-18 · feat(crwu-audit) · 闭环异常前置告警与问题序号（送达规范 v1.5 / renderer 1.2.5）

- **闭环异常联动**：以 `reviewComparison.reviewItems[].inFileResolution=L-unclosed` 为唯一事实源，在首屏结论、
  《人工复核对照》标题下、所属复核级次和具体条目四处同步提示；首屏告警可直接跳到复核区，员工无需在长文中查找。
- **复核标签**：异常条目显示“闭环异常｜称已修复但实际未修改”，对应复核级次摘要显示“实际未落实 N”；
  标签同时使用文字、边框和颜色，在打印及黑白场景仍可识别。
- **连续序号**：AI 问题、人工确认、复核意见和未检查项新增区内 `01/N` 显示序号，帮助员工逐项定位；
  JSON 中原有稳定业务 ID 不变，监控与统计口径不受影响。
- **契约与兼容**：送达规范升至 v1.5、renderer 升至 `renderer/1.2.5`；未新增业务字段，
  `schemaVersion` 保持 `1.1.0`。契约测试覆盖告警位置、层级标签、逐项标签和各区序号。
...
- **背景（实测）**：知识库 `04-监管覆盖/`（覆盖 80.7% 项目）、`03-评估方法/02-收益法|03-资产基础法|04-方法选择/`、
  `06-规则库/清单-M-收益法|资产基础法/` 均有正文却**无任何叶子装配键**；方法轴与覆盖层技能 `pending`
  不等于内容可以不装 → 「库里有、审核读不到」。
- **位置 1 · `crwu-audit/references/08-union-dispatch-rules.md`**：新增 §「方法层与覆盖层的库内装配映射」——
  `canonical 方法标签 / overlay 标签 → 库内层级路径` 的唯一映射表；下载清单在原「叶子装配表 ＋ 公共执行契约」之外，
  **再按本次 `methods[]`/`overlays[]` 命中项追加**（只装命中项、清单外零下载）。`crwu-audit/SKILL.md` 步骤 9 与装配说明同步。
- **位置 2 · `crwu-dev-audit-public-general-standards/references/01-kb-assembly.md`**：新增
  `PUBLIC_BASIS_CURRENCY_CHECK`（单文件 `06-规则库/01-监管规则与口径/依据时效对照表`，`RULE-01-03-001~030`，
  其 `02-review-focus.md` §2 已引用该键却下载不到）与 `PUBLIC_PITFALL_LIBRARY_ROOT`（`06-规则库/易错点库/`，
  此前仅房地产叶子装配 → 其余 11 个资产类型读不到 PIT 条目）。
- **位置 3 · `crwu-audit-skill-maintainer/scripts/check_audit_skill_mappings.py`**：
  ① `watched_prefixes` **9 → 16**（补 2 份新方法清单 + 覆盖层 5 层，消除沉默缺口）；
  ② 装配缺口检查**承认 router 分发规则文件为合法装配来源**（否则真覆盖会被误报）；
  ③ 修 `_parse_box_tree`：接受规范式的**无分支根层行**（`crwu-dws/references/00` §4 的渲染示例中根层为裸名，
  旧实现要求每行都有 `├─`/`└─`，导致根层整体丢弃且层级栈错位——327 节点只解析出 315）；
  ④ `scripts/README.md` 同步装配来源与监视前缀口径。
- **效果**：映射检查器 **error 3 → 0**；`test_audit_skill_maintainer.py` 63 项由 `FAILED(1)` 转 **全绿**；
  装配两跑（受影响 public 装配 22 文件）签名一致。知识库侧另出 5 项 `manual_only` 人工修复单
  （其中 1 项为「资产基础法清单文档内容实为收益法清单」的真错误）。
- **验证**：`kb_tool.py validate` error=0；映射检查器 error=0；router 15/15、dws 12/12、maintainer 63/63；`git diff --check`。
...
- **AI 独立发现**：阶段二复核完成后，在 AI 问题区汇总独立检出总数及高/中/低严重程度分布；`category=B` 的卡片标题区增加醒目标识和人工核验提示，未执行复核时不显示，避免误判为独立发现。
...
- **版本与兼容**：送达规范和样例升至 v1.1，renderer 升至 `renderer/1.2.2`；旧版复核汇总与 `aiScorecard`
